...
class Script():  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(self):
        ...
        # Allowed ZFW range
        self.psx_zfw_min = 180000.0
        self.psx_zfw_max = 290000.0

        # Allowed retardant load range
        self.retardant_load_min = 0.0
        self.retardant_load_max = 74000.0
        ...
        self.retardant_load = self.retardant_load_min

        # The target retardant load (when loading)
        self.retardant_load_target = self.retardant_load_max
    ...
    def message(self, message):
        ...
        For now, this goes to the script stdout. Later we might show
        this on the CDU.
        """
        self.logger.info("MSG: %s", message)
    ...
    def addon_message_handler(self, _, value):  # pylint: disable=too-many-branches
        ...
        if command == "DROP":
            if self.system_armed:
                ...
                self.message("System not armed, cannot drop")
        elif command == "STOP":
            self.dropping = False
        elif command == "ARM":
    ...
    def psx_zfw_change(self, _, value):
        ...
        asyncio.create_task(self.repaint_all_mcdus())
        self.logger.info("PSX ZFW changed to %.1f t", self.psx_zfw / 1000)
    ...
    async def tank_control_coro(self):  # pylint: disable=too-many-branches,too-many-statements
        ...
        try:  # pylint:disable=too-many-nested-blocks
            ...
            while True:
                ...
                if self.oew < self.psx_zfw_min or self.oew > self.psx_zfw_max:
                    self.logger.info(
                        "OEW %.1ft not valid, cannot operate tank system",
                        self.oew / 1000)
                    self.reset_tank_system()
                    continue

                if self.retardant_load > self.retardant_load_max:
                    self.retardant_load = self.retardant_load_max
                    repaint = True

                if self.retardant_load < self.retardant_load_min:
                    self.retardant_load = self.retardant_load_min
                    repaint = True
                ...
                if self.loading and self.loading_hook:
                    self.message("Cannot use pump and HOOK at the same time, stopping pump")
                    self.loading = False
                    repaint = True

                if self.loading and self.dropping:
                    self.message("Cannot load and drop at the same time, resetting system")
                    self.loading = False
                    self.dropping = False
                    self.system_armed = False
                    repaint = True
                ...
                if self.loading:
                    self.message("Retardant load in progress")
                    ...
                    if self.retardant_load >= self.retardant_load_target:
                        ...
                        self.loading = False
                        self.message("Retardant load target reached, stopping load")
                    elif self.retardant_load >= self.retardant_load_max:
                        self.retardant_load = self.retardant_load_max
                        self.loading = False
                        self.message("Retardant tanks full, stopping load")
                    repaint = True
                ...
                if self.dropping:
                    self.message("Retardant drop in progress")
                ...
                if self.tailhook_extended:
                    ...
                    if raw_height is None or raw_pibahealtas is None:
                        ...
                        if tas < __HOOK_SPEED_RANGE__[0]:
                            self.message("Too slow for HOOK")
                            hook_use_possible = False
                        if tas > __HOOK_SPEED_RANGE__[1]:
                            self.message("Too fast for HOOK")
                            hook_use_possible = False
                        if height < __HOOK_HEIGHT_RANGE__[0]:
                            self.message("Too low for HOOK")
                            hook_use_possible = False
                        if height > __HOOK_HEIGHT_RANGE__[1]:
                            self.message("Too high for HOOK")
                            hook_use_possible = False
                        if hook_use_possible:
                            self.message("HOOK loading in progress")
                ...
                if repaint:
                    self.logger.info("Adjusted some data, repainting")
                    asyncio.create_task(self.repaint_all_mcdus())
    ...
    def mcduEvent(self, mcdu, event_type, value=None):  # pylint: disable=too-many-branches,too-many-statements
        ...
        if event_type in ["logon", "resume"]:
            ...
            if value == "CLR":
                ...
                if self.loading:
                    self.message("Cannot arm system while loading")
                else:
                    self.logger.info("ARM/DISARM pressed, state is %s", self.system_armed)
                ...
                self.loading = False
                self.retardant_load = min(self.retardant_load_target, self.retardant_load_max)
                repaint = True
    ...
    async def paintTankerPage(self, mcdu):
        ...
        if self.dropping:
            ...
            mcdu.paint(0, 0, L, W, " I IDENTIFY AS A CL-415 ")
