...
class Frankenrouter():  # pylint: disable=too-many-instance-attributes,too-many-public-methods,too-many-statements
    ...
    async def upstream_connector_task(self, name):  # pylint: disable=too-many-branches,too-many-statements,too-many-locals
        ...
        try:  # pylint: disable=too-many-nested-blocks
            ...
            while True:  # pylint: disable=too-many-nested-blocks
                ...
                await self.send_to_upstream(f"name={self.config.identity.router}:FRANKEN.PY frankenrouter PSX router {self.config.identity.router} in {self.config.identity.simulator} {__VERSION__}")  # pylint: disable=line-too-long
                await self.send_to_upstream(f"clientName={self.config.identity.router}:FRANKEN.PY frankenrouter PSX router {self.config.identity.router} in {self.config.identity.simulator} {__VERSION__}")  # pylint: disable=line-too-long
    ...
    async def api_task(self, name):
        """REST API Task."""
        await RouterWebAPI(self).run(name)
