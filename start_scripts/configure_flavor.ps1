...
foreach ($flavor in $FlavorDefinitions) {
    if ($flavor.Type -eq "bool") {
        ...
        if ($null -ne $v) { $flavor.Default = if ($v.Value) { "on" } else { "off" } }
...
$lines = @("# Auto-generated by configure_flavor.ps1 - do not edit manually.")
foreach ($flavor in $FlavorDefinitions) {
    $val  = $newValues[$flavor.Name]
    $line = if ($flavor.Type -eq "bool") {
        $boolVal = if ($val -eq "on") { '$true' } else { '$false' }
        "`$$($flavor.Name) = $boolVal"
    } elseif ($flavor.Type -eq "codemap") {
