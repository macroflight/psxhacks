...
do {
    ...
    foreach ($flavor in $FlavorDefinitions) {
        ...
        $display = if ($flavor.Type -eq "bool" -and $val -like "use default*") {
            $preDefault = $PreFlavorDefaults[$flavor.Name]
            "default (" + $(if ($preDefault) { "yes" } else { "no" }) + ")"
        } else { $val }
...
foreach ($flavor in $FlavorDefinitions) {
    $val  = $newValues[$flavor.Name]
    $line = if ($flavor.Type -eq "bool") {
        if ($val -like "use default*") { $null }
        else {
            $psVal = if ($val -eq "yes") { '$true' } else { '$false' }
            "`$$($flavor.Name) = $psVal"
        }
    } elseif ($flavor.Type -eq "codemap") {
